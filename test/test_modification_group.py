...
def test_modify_group_header(app):
    ...
    if app.group.count() == 0:
        app.group.create(Group(name="test_group_0", header="header_0", footer="footer_0"))
    old_groups = app.group.get_group_list()
    app.group.modify_first_group(Group(header="header_5"))
    new_groups = app.group.get_group_list()
    assert len(old_groups) == len(new_groups)
...
def test_modify_group_footer(app):
    ...
    if app.group.count() == 0:
        app.group.create(Group(name="test_group_0", header="header_0", footer="footer_0"))
    old_groups = app.group.get_group_list()
    app.group.modify_first_group(Group(footer="footer_9"))
    new_groups = app.group.get_group_list()
    assert len(old_groups) == len(new_groups)
...
def test_modify_some_group(app, db, check_ui):
    ...
    group = Group(name="test_group_3", header="header_3", footer="footer_3")
